...
NS_OBJECT_ENSURE_REGISTERED(TcpCubic25);

/**
 * Get the TypeId which allows users running the simulation to set configurable
 * parameters.
 */
TypeId TcpCubic25::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpCubic")
    .SetParent<TcpSocketBase> ()
    .AddConstructor<TcpCubic25> ();
    /*.AddTraceSource ("CongestionWindowCubic",
                     "The TCP connection's congestion window",
                     MakeTraceSourceAccessor (&TcpCubic25::m_cWnd),
 ...
 * Copy constructor.
 */
TcpCubic25::TcpCubic25 (const TcpCubic25& sock) :
...
int
TcpCubic25::Connect (const Address & address)
{
...
TcpCubic25::Fork (void)
{
  return CopyObject<TcpCubic25> (this);
}
...
void
TcpCubic25::IncreaseWindow (Ptr< TcpSocketState > 	tcb,
                            uint32_t 	segmentsAcked )
{
  NS_LOG_FUNCTION (this << segmentsAcked);
  NS_LOG_DEBUG ("TcpCubic receieved ACK for seq " << segmentsAcked <<
                " cwnd " << tcb->m_cWnd <<
                " ssthresh " << tcb->m_ssThresh);

  // Check for exit condition of fast recovery same as NewReno
  if (m_inFastRec && segmentsAcked < m_recover)
    { // Partial ACK, partial window deflation (RFC2582 sec.3 bullet #5 paragraph 3)
      tcb->m_cWnd += tcb->m_segmentSize - (segmentsAcked - tcb->m_txBuffer->HeadSequence ());
  ...
  if (m_inFastRec && seq < m_recover)
    {// First new ACK after fast recovery, reset cwnd as in NewReno
      m_inFastRec = false;
 ...
 */
uint32_t
TcpCubic25::CubicRoot(uint64_t a)
{
  uint32_t x, b, shift;
  /*
 ...
 */
uint32_t
TcpCubic25::CubicTcpFriendliness(uint32_t cnt)
{
  // Convert the cwnd stored in bytes to a cwnd in segments. Whenever the
  // CUBIC algorithm uses cwnd the value cwndSeg will be used.
  uint32_t cwndSeg = m_cWnd / m_segmentSize;
...
void
TcpCubic25::DupAck (const TcpHeader& t, uint32_t count)
{
  NS_LOG_FUNCTION (this);
  ...
  if (count == m_retxThresh && !m_inFastRec)
    {
      ...
      NS_LOG_INFO ("Dupack dropped cwnd to " << m_cWnd << " and ssthresh to " << m_ssThresh);
